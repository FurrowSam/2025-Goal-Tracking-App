...
def load_data():
    ...
    return deepcopy(progress_table)
...
if view == "Today's Activities":
    ...
    if today in progress_table.index:
        ...
        for i, activity in enumerate(progress_table.columns):
            with cols[i % len(cols)]:
                key = f"{today.strftime('%Y-%m-%d')}_{i}"  # Ensure unique keys
        ...
        # Add save button to save progress
    ...
    st.dataframe(progress_table_display, use_container_width=True)
    ...
    daily_completed_df["Day"] = pd.Categorical(
        daily_completed_df["Date"].dt.day_name(),
